...
if __name__ == '__main__':
    crc16 = Crc32Provider()
    print "We take a message which has the hash 0xae68502f"
    # Thanks to path:
    #   http://blog.w-nz.com/archives/2005/07/15/reversing-crc/#comments
    crc16._hash = crc16.xorOut ^ NumberFromHexadecimal('ae68502f')
    print "Required patch to make the hash 0xdeadbeef:"
    p = crc16.patch(NumberFromHexadecimal('deadbeef'))
    p2 = crc16.patch(NumberFromHexadecimal('deadbeef'))
    print "0x"+binascii.hexlify(p)
    print "0x"+binascii.hexlify(p2)
    print "Applying patch, resulting hash:"
    crc16.update(p)
    print NumberToHexadecimal(crc16.hash)
    print "It works ^_^"
